...
static char nome_processo[] = "Aereo";
...
void decolla(int num_pista){
	long tempo_decollo=getRandom(5,15);
	curr_time=getTime();
	printf("%s , %s : Inizio decollo aereo \nPista usata: %d \nTempo necessario decollo: %d \n", curr_time, nome_processo, noem_pista, tempo_decollo);
...
void preparazioneAereo(){
	...
	long tempo_preparazione=getRandom(3,8)
	curr_time=getTime();
...
int main(int argc, char *argv[]){
    ...
    if(int num_pista = attesaAutorizzazione() != -1){
    	decolla(num_pista);
	}
	else{
